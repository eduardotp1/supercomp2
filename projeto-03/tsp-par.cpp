...
double backtrack_par(std::vector<std::pair<double,double>> points, int idx,
double curr_cost,std::vector<int> &curr_sol,double best_cost,
std::vector<int> &best_seq, std::vector<bool> &usado){
...
int main(){
    ...
    {
        #pragma omp master
        backtrack_par(points,1,0,curr_sol,a,best_sol,usado);
    }
    std::cout << path_dist(best_sol,points) << " 0" << "\n";
